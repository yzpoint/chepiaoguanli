...
//doPost 方法：
//
//处理 POST 请求，将请求转发到 tIndex.jsp 页面。
//使用 request.getRequestDispatcher("/WEB-INF/teacher/tIndex.jsp").forward(request, response); 将请求和响应对象转发到 tIndex.jsp 页面。这样可以确保客户端的 URL 不会改变，并且数据可以从 servlet 传递到 JSP 页面。
//doGet 方法：
//

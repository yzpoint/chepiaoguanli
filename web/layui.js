...
// t 引用了 document 对象。
// n 是一个包含了 modules、status、timeout 和 event 属性的对象，用于管理加载的模块、模块加载状态、超时时间和事件处理。
// 版本定义和获取当前脚本路径：
//
// javascript
// 复制代码
...
//     this.v = "2.5.6"
// };
// 定义了一个构造函数 r，用于设置 Layui 的版本号为 2.5.6。
// javascript
// 复制代码
// var o = function () {
//     var e = t.currentScript ? t.currentScript.src : function () {
...
//     return e.substring(0, e.lastIndexOf("/") + 1)
// }();
// o 是一个自执行函数，用于获取当前脚本的路径。它首先尝试使用 t.currentScript.src 获取当前执行的脚本路径，如果不支持 currentScript，则通过遍历 t.scripts 来获取路径。
// 模块映射和路径定义：
//
// javascript
// 复制代码
...
//     "layui.all": "../layui.all"
// };
// u 是一个包含模块名称和对应路径的映射对象，用于确定每个模块的加载路径。
// 模块定义和加载机制：
//
// javascript
// 复制代码
...
//     // 加载模块的实际处理逻辑
// };
// r.prototype.cache = n 将 n 对象赋值给 cache，用于管理模块加载的缓存和状态。
// r.prototype.define 和 r.prototype.use 分别定义了模块的定义和加载方法，用于动态加载和注册模块。
// 模块加载和依赖管理：
//
// javascript
// 复制代码
...
//     // 加载模块的实际处理逻辑
// };
// r.prototype.use 方法用于加载指定的模块，并处理模块的依赖关系。它可以处理单个模块或者多个模块的加载。
// 其他功能方法：
//
// 还包括了获取样式、动态添加 CSS、获取设备信息、事件处理等方法，这些方法使 Layui 在加载和管理模块的同时，提供了一些实用的工具和功能支持。